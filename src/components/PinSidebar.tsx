...
import type { CenterPoint, PinLocation, User } from "@/types";
...
export function PinSidebar({
  ...
  return (
    <div className="w-80 bg-white border-r border-gray-200 flex flex-col h-screen">
      ...
      <div className="p-4 border-b border-gray-200">
        {!isAddingPin ? (
          <Button onClick={() => setIsAddingPin(true)} className="w-full" size="sm">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-4">
          ...
          <div className="space-y-2">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          />
                          <span className="text-sm font-medium truncate">{pin.title}</span>
                        </div>
                        <div className="text-xs text-gray-500 mb-1">{pin.userName}</div>
                        {pin.description && (
                          <div className="text-xs text-gray-600 mb-2">{pin.description}</div>
                        )}
            ...
            {pins.length === 0 && (
              <div className="text-center text-gray-500 text-sm py-8">まだピンがありません</div>
            )}
          </div>
        </div>
      </div>
